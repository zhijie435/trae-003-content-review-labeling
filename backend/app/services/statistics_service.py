...
class StatisticsService:
    ...
    @staticmethod
    def get_overall_statistics(db: Session) -> schemas.StatisticsOut:
        total_tasks = db.query(func.count(models.AnnotationTask.id)).scalar() or 0

        double_annotated = db.query(func.count(models.Annotation.id)).scalar() or 0
        ...
        latest_inspections = StatisticsService._get_latest_inspection_by_task(db)
        pass_count = 0
        fail_count = 0
        arbitrated_count = 0
        ...
        per_annotator_stats = StatisticsService._get_per_annotator_stats(db)
    ...
    @staticmethod
    def _get_latest_inspection_by_task(db: Session) -> List[models.Inspection]:
        subquery = (
            db.query(
    ...
    @staticmethod
    def _get_per_annotator_stats(db: Session) -> List[Dict[str, Any]]:
        ...
        return per_annotator_stats

    @staticmethod
    def list_annotators(db: Session) -> List[schemas.AnnotatorOut]:
        annotators = db.query(models.Annotator).all()
        return [schemas.AnnotatorOut.model_validate(a) for a in annotators]
